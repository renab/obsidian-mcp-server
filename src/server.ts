...
export class ObsidianMCPServer {
  ...
  private async handleToolCall(
    ...
    if (name === 'get_vault_template') {
      const template = await requireTemplates().get(String(args.templateId));
      return json({ schemaVersion: template.schemaVersion, id: template.id, name: template.name,
        description: template.description, version: template.version, author: template.author,
        tags: template.tags ?? [], requiredVariables: template.requiredVariables ?? [],
        vaultRoot: template.vaultRoot, enabled: template.enabled, files: template.files });
    }
  ...
  async connect(transport: Transport, label: string): Promise<void> {
    ...
    console.error(`obsidian-mcp-server running on ${label}`);
  }
  ...
  async close(): Promise<void> { await this.server.close(); }
}
